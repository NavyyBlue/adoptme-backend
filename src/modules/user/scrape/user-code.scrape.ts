...
export class UserCodeScrape {
   ...
   * @param codigo - The student code to scrape.
   * @returns A promise that resolves to the scraped student data.
   */
  static async scrapeAlumno(codigo: string): Promise<Student> {
   ...
   */
  private static parseAlumno(html: string, baseUrl: string): Student {
    const $ = cheerio.load(html);    
